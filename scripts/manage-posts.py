...
def upload_location_priority(location: str) -> int:
    priorities = {
        "queue": 0,
        "backlog": 1,
        "done": 2,
    }
    return priorities.get(location, 99)


def collapse_upload_posts(posts: list[ManagedPost]) -> tuple[list[ManagedPost], dict[str, list[ManagedPost]]]:
    selected: dict[str, ManagedPost] = {}
    duplicates: dict[str, list[ManagedPost]] = {}
    for post in posts:
        source_id = source_id_for_managed(post)
        current = selected.get(source_id)
        if current is None:
            selected[source_id] = post
            continue
        candidates = [current, post]
        chosen = min(candidates, key=lambda item: upload_location_priority(item.location))
        selected[source_id] = chosen
        duplicates.setdefault(source_id, [])
        for candidate in candidates:
            if candidate not in duplicates[source_id]:
                duplicates[source_id].append(candidate)
    return sorted(selected.values(), key=lambda post: (upload_location_priority(post.location), post.path.name)), duplicates


def choose_upload_source(config: dict[str, Any]) -> ManagedPost | None:
    raw_posts = collect_posts(config, locations={"queue"})
    raw_posts.extend(collect_posts(config, locations={"backlog"}))
    raw_posts.extend(post for post in collect_posts(config, locations={"done"}) if has_local_blogger_link(config, post))
    posts, duplicates = collapse_upload_posts(raw_posts)
    print_section_separator()
    print("Select a post to upload or update:")
    if not posts:
        print_posts([])
        return None
    if duplicates:
        print("Duplicate local copies detected. Showing the active copy per source_id:")
        for duplicate_posts in duplicates.values():
            chosen = min(duplicate_posts, key=lambda item: upload_location_priority(item.location))
            hidden = [post for post in duplicate_posts if post != chosen]
            hidden_text = ", ".join(f"{post.location}:{post.path.name}" for post in hidden)
            print(f"  using {chosen.location}:{chosen.path.name}; hiding {hidden_text}")
    print_posts(posts)
    post = choose_post(posts)
    if post is None:
        return None
    if post.location == "backlog" and not confirm(f"Activate {post.path.name} from backlog and upload it now?", "UPLOAD"):
...
def print_backlog_hint(config: dict[str, Any]) -> None:
    ...
    if backlog_count:
        ...
        print("  Use action 1 to select a backlog post and upload it now.")
        print(f"  Use action 7 only if you want to move one file from {backlog_dir} to {queue_dir} without uploading it.")
...
def menu() -> str:
    print_section_separator()
    print("Actions:")
    print("1. Upload or update one selected queue/backlog/done post using configured publish_mode")
    ...
    print("7. Activate one backlog post for later upload")
...
def print_upload_result(result: dict[str, Any]) -> None:
    print("")
    print("Result:")
    print(f"  Blogger action: {result.get('action')}")
    print(f"  Blogger post:   {result.get('blogger_id')} ({result.get('status')})")
    print(f"  Source file:    {result.get('source_path')}")
    print(f"  Local done:     {result.get('done_path')}")
    archived = result.get("archived_done_path")
    if archived:
        print(f"  Archived done:  {archived}")
    print(f"  Tracking:       {result.get('source_id')}")
...
def main() -> int:
    ...
    while True:
        ...
        if action in {"q", "quit", "exit"}:
            return 0
        if action == "1":
            managed = choose_upload_source(config)
            if managed is None:
                continue
            try:
                print("")
                print(f"Running upload/update for {managed.path.name} from {managed.location}.")
                result = upload_or_update(config, managed)
            ...
            def activate() -> None:
                if confirm(
                    f"Move {managed.path.name} from backlog to queue for a later upload?",
                    "ACTIVATE",
                ):
                    ...
                    if result.get("kept_local_record"):
                        details.append("Kept the existing local Blogger tracking record for this source.")
