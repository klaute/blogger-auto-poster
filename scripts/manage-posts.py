...
def choose_upload_source(config: dict[str, Any]) -> ManagedPost | None:
    queue_posts = collect_posts(config, locations={"queue"})
    if queue_posts:
        print_section_separator()
        print("Select an active queue post:")
        print_posts(queue_posts)
        return choose_post(queue_posts)

    print_section_separator()
    print("No active queue posts found.")
    backlog_posts = collect_posts(config, locations={"backlog"})
    if not backlog_posts:
        print_posts([])
        return None

    print("Select a backlog post to activate and upload now:")
    print_posts(backlog_posts)
    post = choose_post(backlog_posts)
    if post is None:
        return None
    if not confirm(f"Activate {post.path.name} from backlog and upload it now?", "UPLOAD"):
...
def menu() -> str:
    print_section_separator()
    print("Actions:")
    print("1. Upload or update one selected queue/backlog post using configured publish_mode")
        ...
        if action == "1":
            ...
            if managed.location == "backlog":
                try:
                    ...
                    print_result(f"Activated {managed.path.name} for upload at {result['queue_path']}.")
                    managed = ManagedPost(
                        path=Path(result["queue_path"]),
                        location="queue",
                        title=managed.title,
