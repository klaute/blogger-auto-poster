...
def choose_upload_source(config: dict[str, Any]) -> ManagedPost | None:
    posts = collect_posts(config, locations={"queue"})
    posts.extend(post for post in collect_posts(config, locations={"done"}) if has_local_blogger_link(config, post))
    posts.extend(collect_posts(config, locations={"backlog"}))
    print_section_separator()
