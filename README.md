...
To publish a parked post, use action 1 in `scripts/manage-posts.py`. If the active queue is empty, the script lets you select a backlog post, activates it, and uploads it in one flow.
...
To upload a parked draft again, use action 1 in the management script. If the active queue is empty, action 1 can select a backlog post, move it from `posts/queue/backlog/` to `posts/queue/`, clear the local Blogger tracking record for that source, archive any same-named local `posts/done/` duplicate under `posts/done/archive/`, and upload the selected post immediately.
...
For action 1, if `posts/queue` is empty, it also offers files from:
...
1. Upload or update one selected queue/backlog post using the configured `publish_mode`.
...
Action 1 is the normal path for publishing or reposting content. When the queue is empty, it can activate a selected backlog file and upload it immediately. Action 7 is only for preparing a backlog file for a later server cycle or a later manual upload. It moves the selected file to `posts/queue/`, clears stale local tracking for that source, and archives a same-named local `posts/done/` duplicate under `posts/done/archive/`.
