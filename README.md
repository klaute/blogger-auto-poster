...
- `posts/queue/backlog/`: parked drafts. The poster and management script ignore these files.
...
- `posts/queue/backlog/`: parked posts. The server and management script ignore these files.
- `posts/queue/ignore/`: ignored posts or source material.
- `posts/done/`: locally processed posts.

To activate a parked post, move it from `posts/queue/backlog/` to `posts/queue/`.
...
If a Blogger post was deleted externally or through the delete action, the local done file has no valid Blogger post ID anymore. The management script shows it as `not linked`. Such a file cannot be taken offline, published, updated, or deleted remotely. To upload it again, move the Markdown file back to `posts/queue/` and use action 1 in the management script.
...
The script lists Markdown files from:
...
1. Upload or update one selected post using the configured `publish_mode`.
...
7. Delete one linked Blogger post.

Actions 2 to 5 and 7 require an existing Blogger link. If a selected done file is `not linked`, the script stops that action and explains that a new upload is required first.
...
| `scripts/manage-posts.py` | `config/config.yml`, `posts/queue`, `posts/done`, `posts/state.json` | `posts/state.json`, Blogger posts, local file moves | Interactive queue and Blogger post management. |
...
| `scripts/export-public-repo.sh` | current repository files | sanitized target repository | Creates a public export with only allowlisted tool files and one example post. |
...
The internal working repository may contain real article drafts and local runtime files. Do not publish it directly.

Create a sanitized public repository export instead:
...
- this `README.md`
- one neutral example post at `posts/queue/example-post.md`

It does not copy:

- real posts from `posts/queue/backlog`
- ignored posts from `posts/queue/ignore`
- local done or failed posts
- `config/config.yml`
- `credentials.json`
- OAuth tokens
- Pushover tokens
...
The export script initializes a new Git repository and creates an initial commit. To push directly:
...
The script output is intentionally explicit: it prints source, target, remote, copied files, excluded private paths, tracked files, tracked posts, commit ID, and push status.
