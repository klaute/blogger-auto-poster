...
- `posts/queue`
- files in `posts/done`, linked or not linked
- `posts/queue/backlog`
...
If the same article exists in more than one local folder, action 1 groups those copies by `source_id` and shows only the active copy. The priority is `posts/queue/`, then `posts/queue/backlog/`, then linked `posts/done/`. This prevents an older done copy from hiding a newer backlog edit.
