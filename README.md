...
To upload a parked draft again, use the backlog activation action in the management script. It moves the selected Markdown file from `posts/queue/backlog/` to `posts/queue/`, clears the local Blogger tracking record for that source, and archives any same-named local `posts/done/` duplicate under `posts/done/archive/`.
...
7. Activate one backlog post for upload.
...
Action 7 is the explicit bridge from parked draft to active queue. It is useful when a Markdown file exists in `posts/queue/backlog/`, but an older same-named copy also exists in `posts/done/`. The action does not delete the done copy. It archives it to `posts/done/archive/`, clears stale local tracking for that source, and moves the backlog file to `posts/queue/`. After that, action 1 can create a new Blogger draft or update an existing remote post if the tracking marker is still found on Blogger.
