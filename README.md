...
To upload or update content, use action 1 in the management script. For linked files in `posts/done/`, action 1 updates the existing Blogger post. For backlog files, action 1 moves the file from `posts/queue/backlog/` to `posts/queue/`, clears the local Blogger tracking record for that source, archives any same-named local `posts/done/` duplicate under `posts/done/archive/`, and uploads the selected post immediately.
...
Action 1 is the normal path for publishing, reposting, or updating content. It can update a linked done file directly, activate a selected backlog file and upload it immediately, or process an active queue file. Action 7 is only for preparing a backlog file for a later server cycle or a later manual upload. It moves the selected file to `posts/queue/`, clears stale local tracking for that source, and archives a same-named local `posts/done/` duplicate under `posts/done/archive/`.
