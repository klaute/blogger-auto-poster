...
def move_source_to_done(config: dict[str, Any], source_path: Path) -> tuple[Path, str | None]:
    _, done_dir, _ = posting_paths(config)
...
def activate_backlog_post(config: dict[str, Any], managed: ManagedPost) -> dict[str, Any]:
    ...
    posted = state.get("posted_posts") or {}
    previous_record = posted.get(source_id) if isinstance(posted, dict) else None
    ...
    activations[source_id] = {
        ...
        "existing_blogger_id": previous_record.get("blogger_id") if isinstance(previous_record, dict) else None,
    ...
    return {
        ...
        "queue_path": str(target),
        "archived_done_path": archived_done_path,
        "kept_local_record": bool(previous_record),
    }
...
def upload_now(config: dict[str, Any], managed: ManagedPost) -> bool:
    upload_or_update(config, managed)
    return True
...
def upload_or_update(config: dict[str, Any], managed: ManagedPost) -> dict[str, Any]:
    _, _, state_file = posting_paths(config)
    post = read_post(managed.path)
    ...
    link_source = "none"
    try:
        _, post_id, _ = remote_post_id_for_path(config, managed.path)
        link_source = "state_or_remote_marker"
    except RuntimeError:
        post_id = None

    if post_id:
        try:
            blogger_post = update_post(config, post_id, post, tracked_html, labels)
            action = "update"
            detected_existing = True
            updated_existing = True
        except Exception as exc:
            if http_status_code(exc) != 404:
                raise
            blogger_post = insert_post(config, post, tracked_html, labels)
            action = "recreate"
            link_source = "stale_state_or_deleted_remote"
        ...
        detected_existing = False
        updated_existing = False
    blogger_post["labels"] = labels

    done_path, archived_done_path = move_source_to_done(config, managed.path)
    state = load_state(state_file)
    record_posted(
        state,
        source_id,
